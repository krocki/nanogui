...
* @Last Modified time: 2017-03-02 07:29:04
*/
...
void SQGraph::draw ( NVGcontext *ctx ) {
	...
	// border
	// nvgBeginPath ( ctx );
	// nvgRect ( ctx, mPos.x() + 1, mPos.y(), mSize.x(), mSize.y() );
	// nvgFillColor ( ctx, Color ( 0, 0, 0, 64 ) );
	// nvgFill ( ctx );
	
	// squares
	int num_squares = 7;
	float margin_left = 1;
	float margin_right = 47;
	float margin_top = 4.4;
	float margin_bottom = 3.5;
	float spacing = 3;
	...
	for ( int i = 0; i < num_squares; i++ ) {
		...
		float current_val = mValues.block ( i * bin_size, 0, bin_size, 1 ).mean() / ( data_max * 1.5f );
