...
* @Last Modified time: 2017-03-01 11:34:43
*/
...
bool Console::mouseMotionEvent ( const Vector2i &p, const Vector2i & /* rel */,
	...
	if ( !mEditable )
		...
		if ( mSpinnable && !focused() && spinArea ( mMousePos ) != SpinArea::None ) /* scrolling arrows */
							...
							if ( key == GLFW_KEY_BACKSPACE ) {
								...
								if ( key == GLFW_KEY_DELETE ) {
									...
									if ( key == GLFW_KEY_ENTER ) {
										...
										if ( key == GLFW_KEY_A && modifiers == SYSTEM_COMMAND_MOD ) {
											mCursorPos = ( int ) mValueTemp.length();
											mSelectionPos = 0;
										}
										else
											if ( key == GLFW_KEY_X && modifiers == SYSTEM_COMMAND_MOD ) {
												copySelection();
												deleteSelection();
											}
											else
												if ( key == GLFW_KEY_C && modifiers == SYSTEM_COMMAND_MOD )
													copySelection();
												else
													if ( key == GLFW_KEY_V && modifiers == SYSTEM_COMMAND_MOD ) {
														deleteSelection();
														pasteFromClipboard();
													}
...
Console::SpinArea Console::spinArea ( const Vector2i &pos ) {
	if ( 0 <= pos.x() - mPos.x() && pos.x() - mPos.x() < 14.f ) { /* on scrolling arrows */
		if ( mSize.y() >= pos.y() - mPos.y() && pos.y() - mPos.y() <= mSize.y() / 2.f ) /* top part */
			...
			if ( 0.f <= pos.y() - mPos.y() && pos.y() - mPos.y() > mSize.y() / 2.f ) /* bottom part */
