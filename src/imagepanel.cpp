...
int ImagePanel::indexForPosition ( const Vector2i &p ) const {
	...
	bool overImage = pp.x() >= 0.0f && pp.y() >= 0.0f && pp.x() - std::floor ( pp.x() ) < iconRegion &&
...
void ImagePanel::draw ( NVGcontext *ctx ) {
	...
	for ( size_t i = 0; i < mImages.size(); ++i ) {
		...
		if ( mMouseIndex == ( int ) i ) nvgStrokeColor ( ctx, nvgRGBA ( 255, 0, 0, 64 ) );
		else nvgStrokeColor ( ctx, nvgRGBA ( 255, 255, 255, 20 ) );
