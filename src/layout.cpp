	...
	for ( auto w : widget->children() ) {
		...
		switch ( mAlignment ) {
			...
			case Alignment::Fill:
				...
				break;
			case Alignment::Fill2D:
				pos[axis2] += mMargin;
				pos[axis1] += mMargin;
				targetSize[axis2] = fs[axis2] ? fs[axis2] : ( containerSize[axis2] - mMargin * 2 );
				targetSize[axis1] = fs[axis1] ? fs[axis1] : ( containerSize[axis1] - mMargin * 2 );
				break;
		}
...
void GroupLayout::performLayout ( NVGcontext *ctx, Widget *widget ) const {
	float height = mMargin, availableWidth =
	...
	int heightNeeded = 0;
	int heightAvailable = widget->height() - 2 * mMargin + mSpacing;
	...
	for ( auto c : widget->children() ) {
		...
		heightNeeded += c->preferredSize ( ctx ).y();
		heightAvailable -= mSpacing;
	}
	
	float scale = ( float ) heightAvailable / ( float ) heightNeeded;
	...
	for ( auto c : widget->children() ) {
		...
		Vector2i ps = Vector2i ( availableWidth - ( indentCur ? mGroupIndent : 0 ), scale * c->preferredSize ( ctx ).y() );
		...
		height += scale * c->preferredSize ( ctx ).y();
...
Vector2i VGroupLayout::preferredSize ( NVGcontext *ctx, const Widget *widget ) const {
	int height = 2 * mMargin, width = mMargin;
	
	const Window *window = dynamic_cast<const Window *> ( widget );
	if ( window && !window->title().empty() )
		height += widget->theme()->mWindowHeaderHeight + mMargin + mSpacing;
	...
	for ( auto c : widget->children() ) {
		...
		const Label *label = dynamic_cast<const Label *> ( c );
		if ( !first )
			width += ( label == nullptr ) ? mSpacing : mGroupSpacing;
		first = false;
		...
		bool indentCur = indent && label == nullptr;
		width += targetSize.x();
		height = std::max ( height, targetSize.y() + 2 * mMargin + ( indentCur ? mGroupIndent : 0 ) );
...
void VGroupLayout::performLayout ( NVGcontext *ctx, Widget *widget ) const {
	float width = mMargin, availableHeight =
					  ( widget->fixedHeight() ? widget->fixedHeight() : widget->height() ) - 2 * mMargin;
	...
	int widthNeeded = 0;
	int widthAvailable = widget->width() - 2 * mMargin + mSpacing;
	...
	for ( auto c : widget->children() ) {
		...
		widthNeeded += c->preferredSize ( ctx ).x();
		widthAvailable -= mSpacing;
	}
	
	float scale = ( float ) widthAvailable / ( float ) widthNeeded;
	...
	for ( auto c : widget->children() ) {
		...
		Vector2i ps = Vector2i ( scale * c->preferredSize ( ctx ).x(), availableHeight - ( indentCur ? mGroupIndent : 0 ) );
		// Vector2i fs = c->fixedSize();
		...
		c->setPosition ( Vector2i ( width, height + mMargin + ( indentCur ? mGroupIndent : 0 ) ) );
		c->setSize ( ps );
		c->performLayout ( ctx );
		
		width += scale * c->preferredSize ( ctx ).x();
