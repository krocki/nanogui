...
class NANOGUI_EXPORT GLShader {
  ...
  public:
	...
	template <typename Matrix> void uploadAttribWithSize ( const std::string &name, const Matrix &M, size_t num_elements ) {
		...
		uploadAttrib ( name, ( uint32_t ) num_elements, ( int ) M.rows(), compSize,
		               glType, integral, M.data(), version );
	}
